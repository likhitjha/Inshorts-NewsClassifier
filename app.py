...
MNB_Classifier = pickle.load(open('models/MNB_Classifier.pkl', 'rb'))
...
def predict():
    ...
    x=cv.transform([request.form['userNews']]) #Convert string inputs to count vector
    result = MNB_Classifier.predict(x)


    return render_template('index.html', prediction_text='News is {}'.format(result[0]))
